        ...
        NavigationView {
            ZStack {
                ...
                VStack {
                    ...
                    ToolbarModelView(title: "Dashboard", hasBackButton: false, button1Icon: Constants.shared.IMAGE_OPTION_ICON, button2Icon: Constants.shared.IMAGE_FILTER_ICON) { self.presentationMode.wrappedValue.dismiss() }
...
struct ExpenseMainView: View {
    ...
    @AppStorage(Constants.shared.UD_EXPENSE_CURRENCY) var CURRENCY: String = ""
    ...
    private func getTotalBalance() -> String {
        var value = Double(0)
        for i in expense {
            if i.type == Constants.shared.TRANS_TYPE_INCOME { value += i.amount }
            else if i.type == Constants.shared.TRANS_TYPE_EXPENSE { value -= i.amount }
        }
    ...
    var body: some View {
        ...
        ScrollView(showsIndicators: false) {
            ...
            if fetchRequest.wrappedValue.isEmpty {
                LottieView(animationType: .empty_face).frame(width: 300, height: 300)
                VStack {
...
struct ExpenseModelView: View {
    ...
    init(isIncome: Bool, filter: ExpenseCDFilterTime, categTag: String? = nil) {
        ...
        self.type = isIncome ? Constants.shared.TRANS_TYPE_INCOME : Constants.shared.TRANS_TYPE_EXPENSE
        let sortDescriptor = NSSortDescriptor(key: "occuredOn", ascending: false)
        if filter == .all {
    ...
    var body: some View {
        HStack {
            ...
            NavigationLink(destination: NavigationLazyView(ExpenseFilterView(categTag: expenseObj.tag)), label: {
                Image(getTransTagIcon(transTag: TransactionsTag(rawValue: expenseObj.tag ?? "") ?? .TRANS_TAG_OTHERS))
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack {
                    ...
                    Spacer()
                    TextView(text: "\(expenseObj.type == Constants.shared.TRANS_TYPE_INCOME ? "+" : "-")\(CURRENCY)\(expenseObj.amount)", type: .subtitle_1)
                        .foregroundColor(expenseObj.type == Constants.shared.TRANS_TYPE_INCOME ? Color.main_green : Color.main_red)
                }
                HStack {
                    TextView(text: getTransTagTitle(transTag: TransactionsTag(rawValue: expenseObj.tag ?? "") ?? .TRANS_TAG_OTHERS), type: .body_2).foregroundColor(Color.text_primary_color)
                    Spacer()
