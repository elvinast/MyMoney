...
struct ExpenseFilterView: View {
    ...
    func getToolbarTitle() -> String {
        ...
        } else if let tag = categTag { return getTransTagTitle(transTag: TransactionsTag(rawValue: tag) ?? .TRANS_TAG_OTHERS) }
        return "Dashboard"
    }
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack {
                    ToolbarModelView(title: getToolbarTitle(), button1Icon: Constants.shared.IMAGE_FILTER_ICON) { self.presentationMode.wrappedValue.dismiss() }
...
struct ExpenseFilterChartView: View {
    ...
    private func getChartModel() -> [ChartModel] {
        ...
        for i in transactions {
            models.append(ChartModel(transType: getTransTagTitle(transTag: TransactionsTag(rawValue: i.key) ?? .TRANS_TAG_OTHERS), transAmount: i.value))
        }
        return models
    }
...
struct ExpenseFilterTransList: View {
    ...
    init(isIncome: Bool? = nil, filter: ExpenseCDFilterTime, tag: String? = nil) {
        ...
        if filter == .all {
            ...
            if let isIncome = isIncome {
                predicate = NSPredicate(format: "type == %@", (isIncome ? Constants.shared.TRANS_TYPE_INCOME : Constants.shared.TRANS_TYPE_EXPENSE))
            ...
            let predicate: NSPredicate!
            if let isIncome = isIncome {
                predicate = NSPredicate(format: "occuredOn >= %@ AND occuredOn <= %@ AND type == %@", startDate, endDate, (isIncome ? Constants.shared.TRANS_TYPE_INCOME : Constants.shared.TRANS_TYPE_EXPENSE))
            } else if let tag = tag {
