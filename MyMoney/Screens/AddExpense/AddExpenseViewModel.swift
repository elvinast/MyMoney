...
class AddExpenseViewModel: ObservableObject {
    ...
    @Published var selectedType = Constants.shared.TRANS_TYPE_INCOME
    @Published var selectedTag: TransactionsTag = .TRANS_TAG_TRANSPORT
    ...
    init(expenseObj: ExpenseCD? = nil) {
        ...
        if let expenseObj = expenseObj {
            ...
            self.typeTitle = expenseObj.type == Constants.shared.TRANS_TYPE_INCOME ? "Income" : "Expense"
        } else {
            self.amount = ""
        ...
        self.tagTitle = getTransTagTitle(transTag: TransactionsTag(rawValue: expenseObj?.tag ?? "") ?? .TRANS_TAG_OTHERS)
        self.selectedType = expenseObj?.type ?? Constants.shared.TRANS_TYPE_INCOME
        self.selectedTag = TransactionsTag(rawValue: expenseObj?.tag ?? "") ?? .TRANS_TAG_TRANSPORT
    ...
    func getButtText() -> String {
        if selectedType == Constants.shared.TRANS_TYPE_INCOME { return "\(expenseObj == nil ? "ADD" : "EDIT") INCOME" }
        else if selectedType == Constants.shared.TRANS_TYPE_EXPENSE { return "\(expenseObj == nil ? "ADD" : "EDIT") EXPENSE" }
    ...
    func saveTransaction(managedObjectContext: NSManagedObjectContext) {
        ...
        expense.tag = selectedTag.rawValue
